...
// ---- extra sounds for The Scatterbeans ----
{
  // boing: springy rising pitch with wobble
  phase = 0;
  writeWav("boing", make(0.4, (t) => {
    const f = 180 + 520 * Math.min(1, t / 0.12) + 60 * Math.sin(t * 90) * Math.exp(-t * 6);
    phase += (2 * Math.PI * f) / SR;
    return Math.sin(phase) * 0.7 * Math.exp(-t * 7);
  }));
}
{
  // woof: pitched bark with a noisy attack, twice as enthusiastic as needed
  phase = 0;
  let lp = 0;
  writeWav("woof", make(0.32, (t) => {
    const f = 420 - 260 * Math.min(1, t / 0.18);
    phase += (2 * Math.PI * f) / SR;
    const tone = Math.sin(phase) + 0.5 * Math.sin(phase * 2) + 0.25 * Math.sign(Math.sin(phase));
    lp += 0.2 * (rnd() - lp);
    const env = Math.min(1, t / 0.015) * Math.exp(-t * 9);
    return (tone * 0.35 + lp * 1.2 * Math.exp(-t * 30)) * env;
  }));
}
{
  // lick: short wet slurp
  let lp = 0;
  phase = 0;
  writeWav("lick", make(0.28, (t) => {
    const cut = 0.05 + 0.5 * (t / 0.28);
    lp += cut * (rnd() - lp);
    const f = 600 + 900 * (t / 0.28);
    phase += (2 * Math.PI * f) / SR;
    return (lp * 0.8 + Math.sin(phase) * 0.15) * Math.sin(Math.PI * (t / 0.28));
  }));
}
